...
template <typename T> static xent::Color to_xent_color(const T &c) {
  return xent::Color{
...
xent::View build_ui(App &app) {
  auto counter_text = xent::Text{"Count: 0"}.font_size(36).color(
  ...
  auto root =
      xent::VStack{}
          ...
          .expand()
          .add(xent::Text{"Hello, FluXent!"}.font_size(32).color(
              to_xent_color(theme::res().TextPrimary)))
          .add(xent::Text{"A modern Windows UI framework"}.font_size(14).color(
