...
static void demo_add_slider(Demo *d) {
	...
	XentNodeId sld = demo_slider(d, d->root, 0.0f, 100.0f, 50.0f, NULL, NULL);
...
static void demo_add_progress(Demo *d) {
	...
	xent_set_size(d->ctx, flux_create_text(d->ctx, d->store, row, "75%", 12.0f), (XentSize) {35, 20});
	xent_set_size(d->ctx, flux_create_text(d->ctx, d->store, row, "Loading...", 12.0f), (XentSize) {72, 20});
	xent_set_size(d->ctx, flux_create_text(d->ctx, d->store, row, "40%", 12.0f), (XentSize) {35, 20});
	add_divider(d);
}
...
static void demo_add_badge(Demo *d) {
	...
	xent_set_size(d->ctx, flux_create_text(d->ctx, d->store, row, "Dot", 12.0f), (XentSize) {28, 20});
	xent_set_size(d->ctx, demo_badge(d, row, FLUX_BADGE_NUMBER, 3), (XentSize) {16, 16});
	xent_set_size(d->ctx, flux_create_text(d->ctx, d->store, row, "= 3", 12.0f), (XentSize) {28, 20});
	xent_set_size(d->ctx, demo_badge(d, row, FLUX_BADGE_NUMBER, 42), (XentSize) {24, 16});
	xent_set_size(d->ctx, flux_create_text(d->ctx, d->store, row, "= 42", 12.0f), (XentSize) {36, 20});
	...
	xent_set_size(d->ctx, flux_create_text(d->ctx, d->store, row, "Icon", 12.0f), (XentSize) {36, 20});
	add_divider(d);
}
