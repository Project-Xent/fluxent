...
template <typename T> struct Animator
{
  ...
  bool Update(T new_target, double duration_seconds, T *out_value)
  {
    ...
    {
      if (std::abs(new_target - target) > fluxent::config::Animation::ValueEpsilon)
        changed = true;
    }
    else
    {
      ...
      if (!ValuesEqual(target, new_target))
        changed = true;
    }
  ...
  {
    return std::abs(a - b) < fluxent::config::Animation::ValueEpsilon;
  }

  bool ValuesEqual(const Color &a, const Color &b) { return ColorEqualRgba(a, b); }

  template <typename U> bool ValuesEqual(const U &a, const U &b) { return a == b; }
};
