...
public:
  ...
  void RenderToggleButton(const RenderContext &ctx, const xent::ViewData &data, const Rect &bounds,
                          const ControlState &state);
...
private:
  ...
  std::unordered_map<const xent::ViewData *, ButtonBackgroundTransition> transitions_;
  ...
  void DrawElevationBorder(const RenderContext &ctx, const Rect &bounds, float corner_radius,
                           bool is_accent);
  Rect DrawControlSurface(const RenderContext &ctx, const Rect &bounds, float corner_radius,
                          const Color &fill_color, const Color &stroke_color, bool is_accent,
                          const ControlState &state);
};
