...
inline SliderLayout CalculateSliderLayout(const Rect &bounds, float min, float max, float value,
                                          float scale = 1.0f)
{
  SliderLayout layout;
  ...
  layout.track_rect =
      Rect(bounds.x, center_y - SliderTrackHeight * 0.5f, bounds.width, SliderTrackHeight);

  layout.value_rect = Rect(bounds.x, center_y - SliderTrackHeight * 0.5f,
                           layout.thumb_center_x - bounds.x, SliderTrackHeight);

  layout.thumb_rect = Rect(layout.thumb_center_x - outer_radius, center_y - outer_radius,
...
inline CheckBoxLayout CalculateCheckBoxLayout(const Rect &bounds, float size, float gap,
...
inline ScrollViewLayout CalculateScrollViewLayout(float view_w, float view_h, float content_w,
                                                  float content_h, float scroll_x, float scroll_y,
                                                  bool show_h, bool show_v)
{
  ...
  if (show_v)
    effective_w -= ScrollBarSize;
  if (show_h)
    effective_h -= ScrollBarSize;
  ...
  if (show_v)
  {
    ...
    layout.v_bar.track_rect.height =
        (std::max)(0.0f, layout.v_bar.track_rect.height - 2 * ScrollBarPadding);
    ...
    if (layout.v_bar.track_rect.height > 0)
    {
      ...
      layout.v_bar.thumb_rect = Rect(layout.v_bar.bar_rect.x + ScrollBarPadding,
                                     layout.v_bar.track_rect.y + scroll_ratio * track_scrollable,
                                     ScrollBarSize - 2 * ScrollBarPadding, thumb_h);
    }
  }
  ...
  if (show_h)
  {
    ...
    layout.h_bar.track_rect.width =
        (std::max)(0.0f, layout.h_bar.track_rect.width - 2 * ScrollBarPadding);
    ...
    if (layout.h_bar.track_rect.width > 0)
    {
      ...
      layout.h_bar.thumb_rect = Rect(layout.h_bar.track_rect.x + scroll_ratio * track_scrollable,
                                     layout.h_bar.bar_rect.y + ScrollBarPadding, thumb_w,
