...
Color ButtonRenderer::AnimateBackground(const xent::ViewData *key, const Color &target)
{
...
ID2D1SolidColorBrush *ButtonRenderer::GetBrush(ID2D1DeviceContext *d2d, const Color &color)
{
  if (!brush_)
  {
...
static void DrawButtonContent(const RenderContext &ctx, const xent::ViewData &data,
                              const Rect &snapped_bounds, const Color &text_color)
{
  if (!ctx.text)
    return;
  ...
  // Prepare styles
  Color user_text =
      Color(data.text_color.r, data.text_color.g, data.text_color.b, data.text_color.a);
  ...
  content_area.width =
      std::max(0.0f, content_area.width - (data.padding_left + data.padding_right));
  content_area.height =
      std::max(0.0f, content_area.height - (data.padding_top + data.padding_bottom));

  float start_x = content_area.x + (content_area.width - total_content_width) * 0.5f;

  auto d2d = ctx.graphics->GetD2DContext();
  d2d->PushAxisAlignedClip(snapped_bounds.to_d2d(), D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
...
void ButtonRenderer::DrawElevationBorder(const RenderContext &ctx, const Rect &bounds,
                                         float corner_radius, bool is_accent)
{
  ...
  const float thickness = fluxent::config::Render::StrokeWidth;
  ...
  if (need_recreate)
  {
    ...
    d2d->CreateGradientStopCollection(stops, 2, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP,
                                      &collection);
    if (collection)
    {
      d2d->CreateLinearGradientBrush(
          D2D1::LinearGradientBrushProperties(D2D1::Point2F(0, 0),
                                              D2D1::Point2F(0, res.Elevation.Height)),
    ...
    D2D1_GRADIENT_STOP accent_stops[2] = {
        {res.Elevation.GradientStop1, res.ControlStrokeOnAccentSecondary.to_d2d()},
        {res.Elevation.GradientStop2, res.ControlStrokeOnAccentDefault.to_d2d()},
    };
    ComPtr<ID2D1GradientStopCollection> accent_collection;
    d2d->CreateGradientStopCollection(accent_stops, 2, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP,
                                      &accent_collection);
  ...
  const float band_start = flip_y ? (bounds.y + bounds.height - res.Elevation.Height) : bounds.y;
  ...
  D2D1_ROUNDED_RECT stroke_rect =
      D2D1::RoundedRect(D2D1::RectF(left, top, right, bottom), corner_radius, corner_radius);
  d2d->DrawRoundedRectangle(stroke_rect, brush, thickness);
}

void ButtonRenderer::Render(const RenderContext &ctx, const xent::ViewData &data,
                            const Rect &bounds, const ControlState &state)
{
  ...
  float corner_radius =
      data.corner_radius > 0 ? data.corner_radius : fluxent::config::Layout::DefaultCornerRadius;
  Color user_bg = Color(data.background_color.r, data.background_color.g, data.background_color.b,
                        data.background_color.a);
  bool is_accent = (user_bg.a > 0 && ColorEqualRgba(user_bg, res.AccentDefault)) ||
  ...
  const float snap = fluxent::config::Render::PixelSnap;
  const float snapped_x = std::floor(bounds.x * scale_x + snap) / scale_x;
  const float snapped_y = std::floor(bounds.y * scale_y + snap) / scale_y;
  const float snapped_w =
      std::floor((bounds.x + bounds.width) * scale_x + snap) / scale_x - snapped_x;
  const float snapped_h =
      std::floor((bounds.y + bounds.height) * scale_y + snap) / scale_y - snapped_y;
  ...
  const float fill_width = std::max(0.0f, snapped_bounds.width - (fill_inset * 2.0f));
  const float fill_height = std::max(0.0f, snapped_bounds.height - (fill_inset * 2.0f));

  D2D1_ROUNDED_RECT rounded_rect = D2D1::RoundedRect(
      D2D1::RectF(fill_left, fill_top, fill_left + fill_width, fill_top + fill_height), fill_radius,
      fill_radius);
  ...
  if (!is_accent && user_bg.a > 0 && !state.is_disabled)
  {
    ...
    if (state.is_pressed)
    {
      fill_color.a = static_cast<uint8_t>(fill_color.a * fluxent::config::Interaction::PressedAlpha);
    }
    else if (state.is_hovered)
    {
      fill_color.a = static_cast<uint8_t>(fill_color.a * fluxent::config::Interaction::HoverAlpha);
    }
  }
  ...
  else if (stroke_color.a > 0)
  {
    ...
    D2D1_ROUNDED_RECT stroke_rect =
        D2D1::RoundedRect(D2D1::RectF(snapped_bounds.x + half_thick, snapped_bounds.y + half_thick,
                                      snapped_bounds.x + snapped_bounds.width - half_thick,
                                      snapped_bounds.y + snapped_bounds.height - half_thick),
                          corner_radius, corner_radius);
    d2d->DrawRoundedRectangle(stroke_rect, GetBrush(d2d, stroke_color), thickness);
  }
...
void ButtonRenderer::RenderToggleButton(const RenderContext &ctx, const xent::ViewData &data,
                                        const Rect &bounds, const ControlState &state)
{
  seen_.insert(&data);
  auto d2d = ctx.graphics->GetD2DContext();
