...
float CheckBoxRenderer::AnimateCheckState(const xent::ViewData *key, bool is_checked)
{
...
float CheckBoxRenderer::AnimateFloat(const xent::ViewData *key, float target, float duration_ms)
{
  float current;
...
CheckBoxRenderer::CheckBoxVisualState
CheckBoxRenderer::DetermineVisualState(bool is_checked, const ControlState &state)
{
  if (state.is_pressed)
...
void CheckBoxRenderer::RenderCheckBox(const RenderContext &ctx, const xent::ViewData &data,
                                      const Rect &bounds, const ControlState &state)
{
  ...
  float press_scale = AnimateFloat(
      &data,
      state.is_pressed ? fluxent::config::Interaction::PressedScale : 1.0f,
      fluxent::config::Animation::CheckBoxPress * 1000.0f);
  float check_progress = AnimateCheckState(&data, data.is_checked);

  auto layout = CalculateCheckBoxLayout(bounds, res.CheckBox.Size, res.CheckBox.Gap,
                                        res.CheckBox.GlyphFontSize);
  ...
  else
  {
    if (data.is_checked)
    {
      fill = state.is_pressed ? res.AccentTertiary
    ...
    else
    {
      fill = state.is_pressed
                 ? res.ControlFillTertiary
                 : (state.is_hovered ? res.ControlFillSecondary : res.ControlFillDefault);
  ...
  D2D1_ROUNDED_RECT rr =
      D2D1::RoundedRect(box_rect.to_d2d(), res.CheckBox.CornerRadius, res.CheckBox.CornerRadius);

  D2D1_MATRIX_3X2_F old;
  d2d->GetTransform(&old);
  d2d->SetTransform(D2D1::Matrix3x2F::Scale(press_scale, press_scale,
  ...
  if (stroke.a > 0)
  {
    ...
    d2d->DrawRoundedRectangle(rr, brush.Get(), fluxent::config::Render::StrokeWidth);
  }

  if (check_progress > fluxent::config::Render::VisibilityThreshold)
  {
