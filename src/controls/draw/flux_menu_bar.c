 ...
 * to whether its flyout is open (the Selected state, carried as snap->u.menu.is_checked).
...
void flux_draw_menu_bar_item(
  FluxRenderContext const *rc, FluxRenderSnapshot const *snap, FluxRect const *bounds, FluxControlState const *state
	...
	bool                   open = snap->u.menu.is_checked;
	...
	if (!snap->u.menu.label || !rc->text) return;
	FluxTextStyle ts;
	...
	flux_text_draw(rc->text, FLUX_RT(rc), snap->u.menu.label, &ib, &ts);
}
