...
static bool scroll_is_active(FluxRenderSnapshot const *snap) {
	return snap->u.scroll.drag_axis != 0
	    || snap->u.scroll.v_up_pressed
	    || snap->u.scroll.v_dn_pressed
	    || snap->u.scroll.h_lf_pressed
	    || snap->u.scroll.h_rg_pressed;
}

static float scroll_activity_expansion(FluxRenderSnapshot const *snap, double now) {
	double const HOLD = 0.6, FADE = 0.4;
	if (snap->u.scroll.last_activity_time <= 0.0) return 0.0f;

	double dt = now - snap->u.scroll.last_activity_time;
...
static float scroll_hover_expansion(FluxRenderSnapshot const *snap, FluxRect const *bounds) {
	if (!snap->u.scroll.mouse_over) return 0.0f;

	float const EDGE = 24.0f;
	float       dx   = bounds->w - snap->u.scroll.mouse_local_x;
	float       dy   = bounds->h - snap->u.scroll.mouse_local_y;
...
static FluxColor scroll_thumb_color(ScrollDrawContext const *dc, FluxRect const *thumb, int axis) {
	...
	bool      dragged = dc->snap->u.scroll.drag_axis == axis;
...
static void draw_scroll_thumb(ScrollDrawContext const *dc, FluxRect const *bar, FluxRect const *base_thumb, int axis) {
	...
	bool      drag     = dc->snap->u.scroll.drag_axis == axis;
...
static void draw_scroll_vertical(ScrollDrawContext const *dc) {
	...
	ScrollButton up       = scroll_button(&g->v_up_btn, GLYPH_UP, up_hover, dc->snap->u.scroll.v_up_pressed);
	ScrollButton dn       = scroll_button(&g->v_dn_btn, GLYPH_DOWN, dn_hover, dc->snap->u.scroll.v_dn_pressed);
...
static void draw_scroll_horizontal(ScrollDrawContext const *dc) {
	...
	ScrollButton lf       = scroll_button(&g->h_lf_btn, GLYPH_LEFT, lf_hover, dc->snap->u.scroll.h_lf_pressed);
	ScrollButton rg       = scroll_button(&g->h_rg_btn, GLYPH_RIGHT, rg_hover, dc->snap->u.scroll.h_rg_pressed);
...
void flux_draw_scroll_overlay(FluxRenderContext const *rc, FluxRenderSnapshot const *snap, FluxRect const *bounds) {
	...
	FluxScrollGeomInput input
	  = {bounds, snap->u.scroll.content_w, snap->u.scroll.content_h, snap->u.scroll.x, snap->u.scroll.y, expanded};
	flux_scroll_geom_compute(&g, &input);

	bool show_v = g.has_v && snap->u.scroll.v_vis != FLUX_SCROLL_NEVER;
	bool show_h = g.has_h && snap->u.scroll.h_vis != FLUX_SCROLL_NEVER;
	if (!show_v && !show_h) return;

	ScrollMouse mouse
	  = {snap->u.scroll.mouse_local_x + bounds->x, snap->u.scroll.mouse_local_y + bounds->y, snap->u.scroll.mouse_over != 0};
