...
static void draw_button_content(
  FluxRenderContext const *rc, FluxRenderSnapshot const *snap, FluxRect const *sb, FluxColor text_color
	...
	if (flux_color_af(snap->text_color) > 0.0f) actual_text = snap->text_color;
	...
	if (has_icon) icon_w = flux_text_measure(rc->text, icon_utf8, &is, 0).w;

	if (has_text) text_w = flux_text_measure(rc->text, label, &ts, 0).w;
...
static void render_button_common(
  ...
  bool force_accent
	...
	flux_fill_rounded_rect(rc, &fill_rect, fill_radius, colors.fill);
		...
		if (colors.use_elevation) { flux_draw_elevation_border(rc, &sb, radius, colors.is_accent); }
