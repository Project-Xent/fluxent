...
void              flux_draw_checkbox(
  FluxRenderContext const *rc, FluxRenderSnapshot const *snap, FluxRect const *bounds, FluxControlState const *state
	...
	FluxColor stroke       = flux_color_rgba(
	  (unchecked_stroke.rgba >> 24) & 0xff, (unchecked_stroke.rgba >> 16) & 0xff, (unchecked_stroke.rgba >> 8) & 0xff,
	  ( uint8_t ) (stroke_alpha * 255.0f + 0.5f)
	);

	flux_fill_rounded_rect(rc, &box, box_radius, fill);
	if (flux_color_af(stroke) > 0.0f) flux_draw_rounded_rect(rc, &box, box_radius, stroke, 1.0f);

		if (check_progress > 0.001f && rc->text) {
		...
		if (snap->label && snap->label [0] && rc->text) {
			...
			FluxRect text_rect
			  = {bounds->x + box_size + gap, bounds->y, flux_maxf(0.0f, bounds->w - box_size - gap), bounds->h};
