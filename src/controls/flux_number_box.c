...
static void nb_draw_inner_button(
  FluxRenderContext const *rc, FluxRect const *outer, float margin_l, float margin_t, float margin_r, float margin_b,
  float radius, bool hovered, bool pressed, FluxThemeColors const *t
) {
...
static void nb_draw_icon(
  FluxRenderContext const *rc, FluxRect const *outer, float margin_l, float margin_t, float margin_r, float margin_b,
  char const *icon_utf8, float font_size, FluxColor color
) {
	float x = outer->x + margin_l;
	float y = outer->y + margin_t;
	float w = outer->w - margin_l - margin_r;
...
void flux_draw_number_box(
  FluxRenderContext const *rc, FluxRenderSnapshot const *snap, FluxRect const *bounds, FluxControlState const *state
	...
	else if (chrome_hovered) fill = t ? t->ctrl_fill_secondary : flux_color_rgba(249, 249, 249, 0x80);
	   ...
	   ═══════════════════════════════════════════════════════════ */
	{
		...
		if (zone != ZONE_TEXT && !state->focused) text_state.hovered = 0;
	...
	bool node_pressed = state->pressed;
		...
		if (show_delete) {
			...
			nb_draw_inner_button(
			  rc, &del_outer, NB_INNER_MARGIN_L, NB_INNER_MARGIN_T, NB_INNER_MARGIN_R, NB_INNER_MARGIN_B, radius,
			  del_hovered, del_pressed, t
			);
			...
			if (del_pressed) del_icon_color = t ? t->text_tertiary : flux_color_rgba(0, 0, 0, 0x72);
			else del_icon_color = t ? t->text_secondary : flux_color_rgba(0, 0, 0, 0x9e);

			char x_utf8 [4] = {( char ) 0xee, ( char ) 0xa2, ( char ) 0x94, '\0'};
			nb_draw_icon(
			  rc, &del_outer, NB_INNER_MARGIN_L, NB_INNER_MARGIN_T, NB_INNER_MARGIN_R, NB_INNER_MARGIN_B, x_utf8, 12.0f,
			  del_icon_color
			);
		}
	...
	nb_draw_inner_button(
	  rc, &up_outer, NB_UP_MARGIN, NB_UP_MARGIN, NB_UP_MARGIN, NB_UP_MARGIN, radius, up_hovered, up_pressed, t
	);
	nb_draw_inner_button(
	  rc, &dn_outer, NB_DN_MARGIN_L, NB_DN_MARGIN_T, NB_DN_MARGIN_R, NB_DN_MARGIN_B, radius, dn_hovered, dn_pressed, t
	);
	...
	if (!snap->nb_up_enabled) up_icon_color = t ? t->text_disabled : flux_color_rgba(0, 0, 0, 0x5c);
	else if (up_pressed) up_icon_color = t ? t->text_tertiary : flux_color_rgba(0, 0, 0, 0x72);
	else up_icon_color = t ? t->text_secondary : flux_color_rgba(0, 0, 0, 0x9e);

	FluxColor dn_icon_color;
	if (!snap->nb_down_enabled) dn_icon_color = t ? t->text_disabled : flux_color_rgba(0, 0, 0, 0x5c);
	else if (dn_pressed) dn_icon_color = t ? t->text_tertiary : flux_color_rgba(0, 0, 0, 0x72);
	else dn_icon_color = t ? t->text_secondary : flux_color_rgba(0, 0, 0, 0x9e);
	...
	nb_draw_icon(
	  rc, &up_outer, NB_UP_MARGIN, NB_UP_MARGIN, NB_UP_MARGIN, NB_UP_MARGIN, up_utf8, icon_fs, up_icon_color
	);
	nb_draw_icon(
	  rc, &dn_outer, NB_DN_MARGIN_L, NB_DN_MARGIN_T, NB_DN_MARGIN_R, NB_DN_MARGIN_B, dn_utf8, icon_fs, dn_icon_color
	);
}
