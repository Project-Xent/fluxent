...
static uint32_t count_codepoints(const char *src, uint32_t byte_count) {
    uint32_t cp = 0;
...
static int build_mask(const char *src, char *dst, int dst_cap) {
    if (!src || !src[0]) { if (dst && dst_cap > 0) dst[0] = '\0'; return 0; }
    int out = 0;
    const unsigned char *p = (const unsigned char *)src;
    while (*p) {
        int skip;
...
static uint32_t remap_offset(const char *original, uint32_t byte_offset) {
    if (!original) return 0;
    uint32_t cp = count_codepoints(original, byte_offset);
    return cp * 3;
}

void flux_draw_password_box(const FluxRenderContext *rc,
