...
void flux_draw_progress_ring(const FluxRenderContext *rc,
    ...
    if (sweep_rad > pi2 - 0.001f)
        sweep_rad = pi2 - 0.001f;
    ...
    ID2D1GeometrySink_BeginFigure(sink, flux_point(start_x, start_y),
    ...
    arc.arcSize = (sweep_rad > 3.14159265f) ? D2D1_ARC_SIZE_LARGE
                                            : D2D1_ARC_SIZE_SMALL;
    ...
    ID2D1RenderTarget_DrawGeometry(FLUX_RT(rc), (ID2D1Geometry *)path,
                                   (ID2D1Brush *)rc->brush, stroke_w,
                                   stroke_style);
