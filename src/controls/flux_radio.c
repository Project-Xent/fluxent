  ...
  FluxRenderContext const *rc, FluxRenderSnapshot const *snap, FluxRect const *bounds, FluxControlState const *state
	...
	FluxColor outer_fill, outer_stroke, glyph_fill;
		...
		if (!state->enabled) {
			...
			outer_stroke
			  = flux_anim_lerp_color(flux_anim_lerp_color(acc_normal, acc_hover, hover_t), acc_press, press_t);
			...
			outer_fill
			  = flux_anim_lerp_color(flux_anim_lerp_color(fill_normal, fill_hover, hover_t), fill_press, press_t);
	...
	flux_stroke_ellipse(rc, cx, cy, outer_r, outer_r, outer_stroke, 1.0f);
		...
		if (check_progress > 0.01f) {
			...
			float target_r = flux_anim_mixf(
			  flux_anim_mixf(RADIO_GLYPH_NORMAL, RADIO_GLYPH_HOVER, hover_t), RADIO_GLYPH_PRESS, press_t
			);
		...
		if (snap->label && snap->label [0] && rc->text) {
			...
			float const radio_size = outer_r * 2.0f;
			FluxRect    text_rect
			  = {bounds->x + radio_size + gap, bounds->y, flux_maxf(0.0f, bounds->w - radio_size - gap), bounds->h};
