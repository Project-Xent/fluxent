...
void flux_draw_text(const FluxRenderContext *rc,
    ...
    if (flux_color_af(snap->text_color) > 0.0f) {
        ...
        text_color = t ? t->text_primary : flux_color_rgba(0, 0, 0, 0xE4);
    }

    /* Disabled text */
    if (!state->enabled) {
        text_color = t ? t->text_disabled : flux_color_rgba(0, 0, 0, 0x5C);
    }
