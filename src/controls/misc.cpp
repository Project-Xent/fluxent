...
void ControlRenderer::RenderText(const xent::ViewData &data, const Rect &bounds)
{
...
void ControlRenderer::RenderCard(const xent::ViewData &data, const Rect &bounds)
{
  ...
  D2D1_ROUNDED_RECT stroke_rect =
      D2D1::RoundedRect(D2D1::RectF(bounds.x + inset, bounds.y + inset,
                                    bounds.x + bounds.width - inset,
                                    bounds.y + bounds.height - inset),
                                                    corner_radius, corner_radius);
  d2d->DrawRoundedRectangle(stroke_rect, GetBrush(res.CardStrokeDefault),
                            fluxent::config::Render::StrokeWidth);
}
...
void ControlRenderer::RenderDivider(const Rect &bounds, bool horizontal)
{
  ...
  if (horizontal)
  {
    ...
    d2d->DrawLine(D2D1::Point2F(bounds.x, y), D2D1::Point2F(bounds.x + bounds.width, y),
                  GetBrush(res.DividerStrokeDefault), fluxent::config::Render::StrokeWidth);
  }
  else
  {
    float x = bounds.x + bounds.width / 2.0f;
    d2d->DrawLine(D2D1::Point2F(x, bounds.y), D2D1::Point2F(x, bounds.y + bounds.height),
                  GetBrush(res.DividerStrokeDefault), fluxent::config::Render::StrokeWidth);
  }
}

void ControlRenderer::RenderView(const xent::ViewData &data, const Rect &bounds)
{
  auto d2d = graphics_->GetD2DContext();
  ...
  if (data.border_width > 0 && user_border.a > 0)
  {
    ...
    {
      D2D1_ROUNDED_RECT stroke_rect =
          D2D1::RoundedRect(D2D1::RectF(bounds.x + half_border, bounds.y + half_border,
                                        bounds.x + bounds.width - half_border,
                                        bounds.y + bounds.height - half_border),
                            corner_radius, corner_radius);
      d2d->DrawRoundedRectangle(stroke_rect, GetBrush(user_border), data.border_width);
    }
    else
    {
      D2D1_RECT_F rect = D2D1::RectF(bounds.x + half_border, bounds.y + half_border,
                                     bounds.x + bounds.width - half_border,
                                     bounds.y + bounds.height - half_border);
