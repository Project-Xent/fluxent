...
ID2D1SolidColorBrush *ScrollViewRenderer::GetBrush(ID2D1DeviceContext *d2d,
                                                   const Color &color) {
  if (!brush_) {
...
void ScrollViewRenderer::Render(const RenderContext &ctx,
  ...
  // 2. Draw Background
  Color bg_color = Color(data.background_color.r, data.background_color.g,
                         data.background_color.b, data.background_color.a);

  if (bg_color.a > 0) {
    if (data.corner_radius > 0) {
      D2D1_ROUNDED_RECT rr = D2D1::RoundedRect(
          bounds.to_d2d(), data.corner_radius, data.corner_radius);
...
void ScrollViewRenderer::RenderOverlay(const RenderContext &ctx,
  ...
  auto should_show = [](xent::ScrollBarVisibility vis, float content, float size) {
      if (vis == xent::ScrollBarVisibility::Hidden || vis == xent::ScrollBarVisibility::Disabled) return false;
      if (vis == xent::ScrollBarVisibility::Visible) return true;
  ...
  // 3. Calculate Layout
  auto layout = CalculateScrollViewLayout(bounds.width, bounds.height, content_width, content_height,
                                          data.scroll_offset_x, data.scroll_offset_y, show_h, show_v);

  Color thumb_color = state.is_hovered ? res.ControlStrongFillDisabled : res.ControlStrongFillDefault;

  // 4. Render Vertical
  if (layout.v_bar.visible) {
     D2D1_ROUNDED_RECT thumb = D2D1::RoundedRect(
         layout.v_bar.thumb_rect.to_d2d(),
  ...
  if (layout.h_bar.visible) {
     D2D1_ROUNDED_RECT thumb = D2D1::RoundedRect(
         layout.h_bar.thumb_rect.to_d2d(),
