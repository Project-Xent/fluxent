...
static constexpr double kSliderAnimationSeconds = fluxent::config::Animation::Slider;
...
Color SliderRenderer::AnimateColor(const xent::ViewData *key, const Color &target)
{
  Color current;
...
ID2D1SolidColorBrush *SliderRenderer::GetBrush(ID2D1DeviceContext *d2d, const Color &color)
{
  if (!brush_)
  {
...
void SliderRenderer::Render(const RenderContext &ctx, const xent::ViewData &data,
                            const Rect &bounds, const ControlState &state)
{
  ...
  float snapped_w = std::floor((bounds.x + bounds.width) * scale_x + 0.5f) / scale_x - snapped_x;
  float snapped_h = std::floor((bounds.y + bounds.height) * scale_y + 0.5f) / scale_y - snapped_y;
...

  {
    D2D1_ROUNDED_RECT track_rect = D2D1::RoundedRect(
        layout.track_rect.to_d2d(), layout.track_rect.height / 2, layout.track_rect.height / 2);
  ...
  if (layout.thumb_center_x > layout.track_rect.x)
  {
    D2D1_ROUNDED_RECT value_r = D2D1::RoundedRect(
        layout.value_rect.to_d2d(), layout.value_rect.height / 2, layout.value_rect.height / 2);
    d2d->FillRoundedRectangle(value_r, GetBrush(d2d, value_fill));
  }

  {
    D2D1_ELLIPSE outer_ellipse = D2D1::Ellipse(
        D2D1::Point2F(layout.thumb_center_x, layout.thumb_rect.y + layout.thumb_rect.height * 0.5f),
