...
static int ClampIndex(int index, size_t length)
{
  if (index < 0)
    return 0;
...
void TextBoxRenderer::BeginFrame() { needs_redraw_ = false; }

bool TextBoxRenderer::EndFrame() { return needs_redraw_; }

static Microsoft::WRL::ComPtr<ID2D1LinearGradientBrush>
CreateTextBoxElevationBrush(ID2D1DeviceContext *d2d, const theme::ThemeResources &res,
                            bool is_focused, float height)
{
  ...
  d2d->CreateGradientStopCollection(stops, 2, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, &collection);
  if (!collection)
    return nullptr;

  Microsoft::WRL::ComPtr<ID2D1LinearGradientBrush> brush;
  d2d->CreateLinearGradientBrush(
      D2D1::LinearGradientBrushProperties(D2D1::Point2F(0, 0), D2D1::Point2F(0, height)),
      collection.Get(), &brush);

  return brush;
}

void TextBoxRenderer::DrawElevationBorder(const RenderContext &ctx, const Rect &bounds,
                                          float corner_radius, bool is_focused, bool is_hovered)
{
  auto d2d = ctx.graphics->GetD2DContext();
  const auto &res = ctx.Resources();

  D2D1_ROUNDED_RECT rr = D2D1::RoundedRect(bounds.to_d2d(), corner_radius, corner_radius);
  ...
  d2d->CreateLinearGradientBrush(
      D2D1::LinearGradientBrushProperties(D2D1::Point2F(0, bounds.y + bounds.height),
  ...
  if (is_focused)
  {
    ...
    d2d->PushLayer(D2D1::LayerParameters(D2D1::InfiniteRect(), clip_geom.Get()), nullptr);

    float line_y = bounds.y + bounds.height - 1.0f;
    d2d->DrawLine(D2D1::Point2F(bounds.x, line_y), D2D1::Point2F(bounds.x + bounds.width, line_y),
                  accent_brush.Get(), 2.0f);
                                 ...
                                 const ControlState &state, float corner_radius)
{
  ...
  {
    DrawElevationBorder(ctx, bounds, corner_radius, state.is_focused, state.is_hovered);
  }

  return bounds;
}

static Rect ComputeTextBounds(const xent::View &data, const Rect &bounds,
                              const theme::ThemeResources &res)
{
  ...
  return Rect(bounds.x + pad_l, bounds.y + pad_t, std::max(0.f, bounds.width - pad_l - pad_r),
              std::max(0.f, bounds.height - pad_t - pad_b));
}

static void BuildDisplayText(const xent::View &data, std::wstring &display_text,
                             std::wstring &composition_text, int &composition_start)
{
  std::wstring wtext = ToWide(data.text_content);
  composition_text = ToWide(data.composition_text);
  display_text = wtext;

  composition_start = std::min(data.cursor_start, data.cursor_end);
  composition_start = ClampIndex(composition_start, display_text.size());

  if (!composition_text.empty())
  {
    display_text.insert(composition_start, composition_text);
  }
}

static TextStyle BuildTextStyle(const xent::View &data, const theme::ThemeResources &res,
                                bool disabled)
{
  TextStyle style;
  style.font_size = data.font_size > 0 ? data.font_size : kControlContentThemeFontSize;
  style.color = disabled ? res.TextDisabled : res.TextPrimary;
  style.word_wrap = false;
  return style;
}

void TextBoxRenderer::Render(const RenderContext &ctx, const xent::View &data, const Rect &bounds,
                             const ControlState &state)
{
  const auto &res = ctx.Resources();
  float corner_radius = data.corner_radius > 0 ? data.corner_radius : res.ControlCornerRadius;

  DrawChrome(ctx, bounds, state, corner_radius);
  Rect text_bounds = ComputeTextBounds(data, bounds, res);
  ...
  BuildDisplayText(data, display_text, wcomposition, composition_start);

  TextStyle style = BuildTextStyle(data, res, state.is_disabled);
  ...
  if (display_text.empty() && !data.placeholder_text.empty())
  {
    ...
    std::wstring wph = ToWide(data.placeholder_text);
  ...
  if (wcomposition.empty() && !data.text_content.empty() && data.cursor_start != data.cursor_end)
  {
    ...
    auto rects =
        text_renderer->GetSelectionRects(display_text, style, text_bounds.width, start, end);

    auto sel_brush = CreateBrush(ctx.graphics->GetD2DContext(), res.AccentDefault);
    if (sel_brush)
      sel_brush->SetOpacity(0.4f);

    for (const auto &r : rects)
    {
      Rect abs_r(text_bounds.x + r.x - data.scroll_offset_x, text_bounds.y + r.y, r.width,
                 r.height);
      if (sel_brush)
      {
        ctx.graphics->GetD2DContext()->FillRectangle(abs_r.to_d2d(), sel_brush.Get());
      }
    }
  }
  ...
  {
    auto rects = text_renderer->GetSelectionRects(
        display_text, style, text_bounds.width, composition_start,
        composition_start + static_cast<int>(wcomposition.length()));
    ...
    {
      D2D1_POINT_2F p1 =
          D2D1::Point2F(text_bounds.x + r.x - data.scroll_offset_x, text_bounds.y + r.y + r.height);
      D2D1_POINT_2F p2 = D2D1::Point2F(text_bounds.x + r.x + r.width - data.scroll_offset_x,
                                       text_bounds.y + r.y + r.height);
      if (underline_brush)
  ...
  if (state.is_focused && wcomposition.empty())
  {
    ...
    if (caret_brush)
    {
      ctx.graphics->GetD2DContext()->DrawLine(D2D1::Point2F(caret_x, caret_y),
  ...
  if (state.is_focused && (wcomposition.empty() ? (data.cursor_start == data.cursor_end) : true))
  {
    ...
    {
      int caret_pos_idx =
          wcomposition.empty() ? data.cursor_end : (composition_start + data.composition_cursor);
      caret_pos_idx = ClampIndex(caret_pos_idx, display_text.size());

      Rect caret =
          text_renderer->GetCaretRect(display_text, style, text_bounds.width, caret_pos_idx);
      float h = caret.height > 0
                    ? caret.height
                    : style.font_size * fluxent::config::Input::DefaultLineHeightMultiplier;
