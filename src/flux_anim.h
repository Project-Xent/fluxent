 ...
 * @return true if ANY channel is still animating (caller should
 *         request another render frame).
 */
static bool inline flux_tween_update_states(
  FluxTween *hover, FluxTween *press, FluxTween *focus, FluxTween *check, bool is_hovered, bool is_pressed,
  bool is_focused, bool is_checked, double now
) {
	bool  active = false;
	float v;
	active |= flux_tween_update(hover, is_hovered ? 1.0f : 0.0f, FLUX_ANIM_DURATION_NORMAL, now, &v);
	active |= flux_tween_update(press, is_pressed ? 1.0f : 0.0f, FLUX_ANIM_DURATION_PRESS, now, &v);
	active |= flux_tween_update(focus, is_focused ? 1.0f : 0.0f, FLUX_ANIM_DURATION_NORMAL, now, &v);
	active |= flux_tween_update(check, is_checked ? 1.0f : 0.0f, FLUX_ANIM_DURATION_FAST, now, &v);
	return active;
}
