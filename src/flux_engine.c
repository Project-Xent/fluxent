...
static bool flux_command_buffer_push(FluxCommandBuffer *buf, FluxRenderCommand const *cmd) {
		if (buf->count == buf->capacity) {
			...
			FluxRenderCommand *new_cmds
			  = ( FluxRenderCommand * ) realloc(buf->cmds, sizeof(FluxRenderCommand) * new_cap);
		...
		for (uint32_t i = 0; i < eng->commands.count; i++) {
			FluxRenderCommand const *cmd = &eng->commands.cmds [i];
				...
				if (cmd->clip_action == FLUX_CLIP_PUSH) {
					...
					if (transform_top < MAX_TRANSFORM_DEPTH) transform_stack [transform_top++] = current;

					/* Push axis-aligned clip */
					D2D1_RECT_F clip
					  = {cmd->bounds.x, cmd->bounds.y, cmd->bounds.x + cmd->bounds.w, cmd->bounds.y + cmd->bounds.h};
					...
					translate._21 = 0.0f;
					translate._22 = 1.0f;
					translate._31 = -cmd->scroll_x;
					translate._32 = -cmd->scroll_y;
			...
			if (cmd->phase == FLUX_PHASE_MAIN) flux_dispatch_render(rc, &cmd->snapshot, &cmd->bounds, &cmd->state);
			else flux_dispatch_render_overlay(rc, &cmd->snapshot, &cmd->bounds);
		}
}
