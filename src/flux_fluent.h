...
#define FLUX_CORNER_RADIUS         4.0f
#define FLUX_STROKE_WIDTH          1.0f
#define FLUX_PIXEL_SNAP            0.5f
#define FLUX_FONT_SIZE_DEFAULT     14.0f
#define FLUX_VISIBILITY_THRESHOLD  0.01f
...
#define FLUX_ANIM_FAST             0.083
#define FLUX_ANIM_NORMAL           0.167
#define FLUX_ANIM_SLOW             0.250
#define FLUX_ANIM_VALUE_EPS        0.001f

static inline FluxColor ft_ctrl_fill_default(void)          { return flux_color_rgba(255,255,255, 0xB3); }
static inline FluxColor ft_ctrl_fill_secondary(void)        { return flux_color_rgba(249,249,249, 0x80); }
static inline FluxColor ft_ctrl_fill_tertiary(void)         { return flux_color_rgba(249,249,249, 0x4D); }
static inline FluxColor ft_ctrl_fill_disabled(void)         { return flux_color_rgba(249,249,249, 0x4D); }
static inline FluxColor ft_ctrl_fill_input_active(void)     { return flux_color_rgba(255,255,255, 0xFF); }

static inline FluxColor ft_ctrl_alt_fill_secondary(void)    { return flux_color_rgba(0,0,0, 0x06); }
static inline FluxColor ft_ctrl_alt_fill_tertiary(void)     { return flux_color_rgba(0,0,0, 0x0F); }
...
static inline FluxColor ft_ctrl_stroke_on_accent_secondary(void) { return flux_color_rgba(255,255,255, 0x14); }
static inline FluxColor ft_ctrl_strong_stroke_default(void) { return flux_color_rgba(0,0,0, 0x9C); }
static inline FluxColor ft_ctrl_strong_stroke_disabled(void){ return flux_color_rgba(0,0,0, 0x37); }

static inline FluxColor ft_ctrl_solid_fill_default(void)    { return flux_color_rgba(255,255,255, 0xFF); }

static inline FluxColor ft_ctrl_strong_fill_default(void)   { return flux_color_rgba(0,0,0, 0x9C); }
static inline FluxColor ft_ctrl_strong_fill_disabled(void)  { return flux_color_rgba(0,0,0, 0x51); }

static inline FluxColor ft_accent_default(void)             { return flux_color_rgb(0,120,212); }
static inline FluxColor ft_accent_secondary(void)           { return flux_color_rgba(0,120,212, 0xE6); }
static inline FluxColor ft_accent_tertiary(void)            { return flux_color_rgba(0,120,212, 0xCC); }
static inline FluxColor ft_accent_disabled(void)            { return flux_color_rgba(0,0,0, 0x37); }

static inline FluxColor ft_text_primary(void)               { return flux_color_rgba(0,0,0, 0xE4); }
static inline FluxColor ft_text_secondary(void)             { return flux_color_rgba(0,0,0, 0x9E); }
static inline FluxColor ft_text_disabled(void)              { return flux_color_rgba(0,0,0, 0x5C); }
static inline FluxColor ft_text_on_accent_primary(void)     { return flux_color_rgb(255,255,255); }
static inline FluxColor ft_text_on_accent_secondary(void)   { return flux_color_rgba(255,255,255, 0xB3); }
static inline FluxColor ft_text_on_accent_disabled(void)    { return flux_color_rgb(255,255,255); }

static inline FluxColor ft_card_bg_default(void)            { return flux_color_rgba(255,255,255, 0xB3); }
static inline FluxColor ft_card_stroke_default(void)        { return flux_color_rgba(0,0,0, 0x0F); }
static inline FluxColor ft_divider_stroke_default(void)     { return flux_color_rgba(0,0,0, 0x14); }

static inline FluxColor ft_focus_stroke_outer(void)         { return flux_color_rgba(0,0,0, 0xE4); }
static inline FluxColor ft_focus_stroke_inner(void)         { return flux_color_rgb(255,255,255); }

static inline FluxColor ft_transparent(void)                { return flux_color_rgba(0,0,0, 0); }

static inline float flux_clamp01(float v) {
    if (v < 0.0f) return 0.0f;
    if (v > 1.0f) return 1.0f;
    return v;
}

static inline float flux_clampf(float v, float lo, float hi) {
    if (v < lo) return lo;
    if (v > hi) return hi;
    return v;
}

static inline float flux_maxf(float a, float b) { return a > b ? a : b; }
static inline float flux_minf(float a, float b) { return a < b ? a : b; }

static inline float flux_srgb_to_linear(float c) {
    if (c <= 0.04045f) return c / 12.92f;
    return powf((c + 0.055f) / 1.055f, 2.4f);
}

static inline float flux_linear_to_srgb(float c) {
    if (c <= 0.0031308f) return 12.92f * c;
    return 1.055f * powf(c, 1.0f / 2.4f) - 0.055f;
}

static inline FluxColor flux_lerp_color_srgb(FluxColor a, FluxColor b, float t) {
    t = flux_clamp01(t);
    ...
    return flux_color_rgba((uint8_t)(sr * 255.0f + 0.5f),
                           (uint8_t)(sg * 255.0f + 0.5f),
                           (uint8_t)(sb * 255.0f + 0.5f),
                           (uint8_t)(sa * 255.0f + 0.5f));
}

static inline FluxRect flux_snap_bounds(const FluxRect *bounds, float sx, float sy) {
...
static inline void flux_draw_focus_rect(const FluxRenderContext *rc,
                                         const FluxRect *bounds, float radius) {
    FluxRect outer = {
        bounds->x - FLUX_FOCUS_OUTER_PAD,
        bounds->y - FLUX_FOCUS_OUTER_PAD,
        bounds->w + FLUX_FOCUS_OUTER_PAD * 2.0f,
    ...
    FluxRect inner = {
        bounds->x - FLUX_FOCUS_INNER_PAD,
        bounds->y - FLUX_FOCUS_INNER_PAD,
        bounds->w + FLUX_FOCUS_INNER_PAD * 2.0f,
...
static inline FluxSliderLayout flux_calc_slider_layout(const FluxRect *bounds,
...
static inline FluxCheckboxLayout flux_calc_checkbox_layout(const FluxRect *bounds,
                                                            float size, float gap,
                                                            float glyph_font_size) {
...
static inline bool flux_scroll_should_show(int vis, float content, float viewport) {
    if (vis == FLUX_SCROLL_NEVER) return false;
    if (vis == FLUX_SCROLL_ALWAYS) return true;
    return content > viewport + FLUX_SCROLL_VIS_EPS;
}

static inline FluxScrollViewLayout flux_calc_scroll_layout(float view_w, float view_h,
...
static inline void flux_draw_elevation_border(const FluxRenderContext *rc,
                                               const FluxRect *bounds,
                                               float radius, bool is_accent) {
    float thickness = FLUX_STROKE_WIDTH;
    float half = thickness * 0.5f;

    /* Resolve gradient colors from theme */
    FluxColor secondary = is_accent
        ? (rc->theme ? rc->theme->ctrl_stroke_on_accent_secondary
                     : ft_ctrl_stroke_on_accent_secondary())
        : (rc->theme ? rc->theme->ctrl_stroke_secondary
                     : ft_ctrl_stroke_secondary());
    FluxColor primary = is_accent
        ? (rc->theme ? rc->theme->ctrl_stroke_on_accent_default
                     : ft_ctrl_stroke_on_accent_default())
        : (rc->theme ? rc->theme->ctrl_stroke_default
                     : ft_ctrl_stroke_default());
    ...
    ID2D1RenderTarget_DrawRoundedRectangle(
        FLUX_RT(rc), &rr, (ID2D1Brush *)grad, thickness, NULL);
