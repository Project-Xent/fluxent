...
static bool rc_needs_grow(const FluxRenderCache *cache) {
...
static FluxCacheSlot *rc_find(FluxRenderCache *cache, uint64_t key) {
    ...
    for (;;) {
        ...
        if (s->tag == FLUX_RC_EMPTY)
            return NULL;
        if (s->tag == FLUX_RC_OCCUPIED && s->key == key)
            return s;
...
FluxCacheEntry *flux_render_cache_get_or_create(FluxRenderCache *cache, uint64_t node_id) {
    ...
    if (cache->count >= cache->max_entries)
        flux_render_cache_evict_lru(cache, 120);
...
ID2D1SolidColorBrush *flux_render_cache_brush(FluxRenderCache *cache,
    ...
    if (!cache || !dc || !cached_rgba || !slot)
        return NULL;

    if (*slot && *cached_rgba == color.rgba) {
        return *slot;
    }
    ...
    bp.opacity = 1.0f;
    bp.transform._11 = 1.0f; bp.transform._12 = 0.0f;
    bp.transform._21 = 0.0f; bp.transform._22 = 1.0f;
    bp.transform._31 = 0.0f; bp.transform._32 = 0.0f;
