...
struct FluxTextRenderer {
	...
	ID2D1RenderTarget    *brush_rt; /* RT the brush was created on */
};
...
static wchar_t *utf8_to_wbuf(char const *utf8, wchar_t *stack, int stack_cap, int *out_len) {
...
static IDWriteTextLayout *get_or_create_layout(
  FluxTextRenderer *tr, wchar_t const *wtext, int wlen, char const *utf8_text, FluxTextStyle const *s, float max_w,
  float max_h
) {
	...
	HRESULT hr = IDWriteFactory_CreateTextLayout(tr->factory, wtext, ( UINT32 ) wlen, fmt, max_w, max_h, &layout);
...
static bool dwrite_measure(
  ...
  XentMeasureMode mode, XentTextMetrics *out
	...
	IDWriteTextLayout *layout = NULL;
	hr = IDWriteFactory_CreateTextLayout(tr->factory, wtext, text_len, fmt, layout_width, 100000.0f, &layout);
...
void flux_text_draw(
  FluxTextRenderer *tr, ID2D1RenderTarget *rt, char const *text, FluxRect const *bounds, FluxTextStyle const *style
	...
	IDWriteTextLayout *layout = get_or_create_layout(tr, wtext, wlen, text, style, bounds->w, bounds->h);
		if (!layout) {
	...
	cf.a = flux_color_af(style->color);
		...
		/* Reuse shared brush — recreate when the render target changes
	       (e.g. main window vs popup use different D2D devices). */
...
FluxSize flux_text_measure(FluxTextRenderer *tr, char const *text, FluxTextStyle const *style, float max_width) {
	...
	IDWriteTextLayout *layout = get_or_create_layout(tr, wtext, wlen, text, style, mw, FLUX_TEXT_UNBOUNDED);
		if (!layout) {
			free_wbuf(wtext, stack);
			return result;
		}
...
flux_text_caret_rect(FluxTextRenderer *tr, char const *text, FluxTextStyle const *style, float max_width, int index) {
	...
	char const *safe = (text && text [0]) ? text : " ";
	wchar_t     stack [256];
	int         wlen;
	...
	float              mw     = max_width > 0.0f ? max_width : FLUX_TEXT_UNBOUNDED;
	IDWriteTextLayout *layout = get_or_create_layout(tr, wtext, wlen, safe, style, mw, FLUX_TEXT_UNBOUNDED);
		if (!layout) {
			free_wbuf(wtext, stack);
			return result;
		}
...
uint32_t flux_text_selection_rects(
  ...
  FluxRect *out_rects, uint32_t max_rects
	...
	UINT32 count = 0;
	IDWriteTextLayout_HitTestTextRange(layout, ( UINT32 ) start, ( UINT32 ) (end - start), 0, 0, NULL, 0, &count);
		if (count == 0) {
			/* Do NOT release layout — cache owns it */
			free_wbuf(wtext, stack);
			return 0;
	...
	DWRITE_HIT_TEST_METRICS *htm = ( DWRITE_HIT_TEST_METRICS * ) malloc(sizeof(DWRITE_HIT_TEST_METRICS) * count);
		if (!htm) {
			free_wbuf(wtext, stack);
			return 0;
		}

	IDWriteTextLayout_HitTestTextRange(layout, ( UINT32 ) start, ( UINT32 ) (end - start), 0, 0, htm, count, &count);
