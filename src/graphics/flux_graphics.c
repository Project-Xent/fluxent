...
static HRESULT create_window_resources(FluxGraphics *gfx) {
	...
	hr = IDCompositionDevice_CreateTargetForHwnd(gfx->dcomp_device, gfx->hwnd, TRUE, &gfx->dcomp_target);
...
void flux_graphics_present(FluxGraphics *gfx, bool vsync) {
	...
	/* For DirectComposition swap chains, vsync is handled by the compositor.
	 * Using Present(1, 0) would block the message loop unnecessarily,
	 * causing input lag during touchpad scrolling. Always use interval=0. */
	UINT interval = 0;
	UINT flags    = 0;
	( void ) vsync; /* DComp handles sync; ignore caller's preference */
...
HRESULT flux_graphics_handle_device_change(FluxGraphics *gfx) {
	...
	if (gfx->hwnd) hr = create_window_resources(gfx);
